...
const ToggleSwitch = () => {
  ...
  useEffect(() => {
    const newTab =  pathname.replace(/\//g, ""); ;//? "search" : "chat";
    if (pathname === "/search") router.push(newTab);
    if (pathname === "/chat")  router.push(newTab);
    if (pathname === "/archive")  router.push(newTab);
  ...
  const handleTabChange = (tab: string) => {
    ...
    if (settings?.isMobile && window) {
      ...
      if (tab === "archive")  router.push("/archive");
    }
  };

  /* <div
        className={`absolute mobile:mt-8 top-1 bottom-1 ${
          activeTab === "chat" ? "w-[30%]" : "w-[35%]"
